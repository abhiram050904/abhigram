...
io.on('connection', async (socket) => {
    ...
    try {
        ...
        socket.on('typing_status', (data) => {
            ...
            const receiverSocketId = activeUsers.get(receiverId);
            if (receiverSocketId) {
                // Get sender's username for the typing indicator
            ...
            try {
                ...
                if (message.isGroupMessage) {
                    ...
                    await Message.findByIdAndUpdate(messageId, { 
                        read: [{ user: userId, readAt: new Date() }]
                    });
        ...
        socket.on('file_uploaded', async (data) => {
            try {
                const { messageId, type, groupId, receiverId } = data;
                
                // Get the uploaded message
                const message = await Message.findById(messageId)
                    .populate('sender', 'username profilePhoto')
                    .populate('read.user', 'username');
                ...
                if (type === 'group') {
                    // Broadcast file message to group members (except sender)
